...
class CovBlock(nn.Module):
    def __init__(self, feature_dimension, features_num, hidden_dim1, hidden_dim2, dropout=0.05):
        super().__init__()

        self.cov_mlp = nn.Sequential(
            nn.Linear(feature_dimension, hidden_dim1),
            nn.Dropout(dropout, inplace=True),
            nn.LeakyReLU(inplace=True),
            nn.Linear(hidden_dim1, hidden_dim2),
            nn.LeakyReLU(inplace=True),
            nn.Linear(hidden_dim2, features_num),
        )
...
class BandSelectBlock(nn.Module):
    def __init__(self, feature_dimension, features_num):
        ...
        for _ in range(features_num):
            self.CovBlockList.append(CovBlock(feature_dimension, 1, feature_dimension, round(feature_dimension * 0.6)))

        self.global_covblock = CovBlock(features_num, 1, features_num * 2, features_num, 0)
