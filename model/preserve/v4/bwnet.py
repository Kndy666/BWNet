...
class BWNet(nn.Module):
    ...
    def forward(self, ms, pan):
        ...
        x = x + skip0
        skip1 = x
        output0 = self.to_hrms0(x + skip0)

        x = self.res2(x)
        skip2 = x
        output1 = self.to_hrms1(x + skip0 + skip1)

        x = self.res3(x)
        skip3 = x
        output2 = self.to_hrms2(x + skip0 + skip1 + skip2)

        x = self.res4(x)
        output3 = self.to_hrms(x + skip0 + skip1 + skip2 + skip3)
